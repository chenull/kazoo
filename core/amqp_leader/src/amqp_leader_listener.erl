...
init([Name]) ->
    kz_util:put_callid(kapi_leader:queue(Name, kz_types:kz_node())),
...
handle_info(_Info, State) ->
    lager:warning("~s unhandled info ~p", [kz_types:kz_node(), _Info]),
    {'noreply', State}.
...
handle_event(JObj, #state{name = Name}) ->
    kz_util:put_callid(kapi_leader:queue(Name, kz_types:kz_node())),
    NodeBin = kz_term:to_binary(kz_types:kz_node()),
...
ready_when_node_is_up() ->
    case kz_nodes:is_up(kz_types:kz_node()) of
        'false' ->
